...
def select(profiles):
 ...
 v=input('Perfil [X cancela]: ').strip().casefold();return names[int(v)-1] if v.isdigit() and 1<=int(v)<=len(names) else None
def configure(root):
 p=file(root)
 while True:
  d=load(p);profiles=d['profiles'];print('\n══ ONMYŌJI · CLOUDFLARE ══\n  1. Criar perfil\n  2. Editar perfil\n  3. Excluir perfil\n  X. Voltar');a=input('Opção: ').strip().casefold()
  if a in {'x','\x1b'}:return
  if a=='1':
   n=input('Nome [X cancela]: ').strip()
   if not n or n.casefold()=='x' or n in profiles:print('Nome inválido ou existente.');continue
   profiles[n]={k:(input(f'{l} [{x}]: ').strip() or x) for k,l,x in FIELDS};save(p,d)
  elif a in {'2','3'}:
   if not profiles:print('Não há perfis configurados.');continue
   print('Perfis:');n=select(profiles)
   if not n:continue
   if a=='3':
    if input(f'Digite EXCLUIR para remover {n}: ').strip()=='EXCLUIR':del profiles[n];save(p,d)
    else:print('Operação cancelada.')
   else:
    q=profiles[n];[print(f'  {i}. {l}: {q.get(k,"")}') for i,(k,l,_) in enumerate(FIELDS,1)];v=input('Editar [X volta]: ').strip()
    if v.isdigit() and 1<=int(v)<=len(FIELDS):k,l,_=FIELDS[int(v)-1];x=input(f'{l} [X cancela]: ').strip();q[k]=q[k] if not x or x.casefold()=='x' else x;save(p,d)
