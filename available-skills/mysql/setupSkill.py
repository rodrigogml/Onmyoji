...
PROFILE_FIELDS=(Field('host','Host','127.0.0.1',True),Field('database','Banco',required=True),Field('user','Usuário',''),Field('vault_profile','Perfil KeePass',required=True),Field('vault_entry_path','Entrada KeePass',required=True),Field('username_field','Campo do usuário','username'),Field('password_field','Campo da senha','password'))
...
def main():
 ...
 screen('MySQL','Configuração','Perfil local da integração');note(f"Use {root/'configs/mysql.toml'} a partir do modelo {SKILL/'configs/mysql.toml.model'}.")
