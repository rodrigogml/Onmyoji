...
DEFAULT_FIELDS=(Field('executable','Executável MySQL','mysql',True),Field('port','Porta',3306,True),Field('connect_timeout','Timeout de conexão (segundos)',10,True),Field('timeout_seconds','Timeout de execução (segundos)',120,True),Field('allow_client_commands','Permitir comandos do cliente',True,True))
PROFILE_FIELDS=(Field('executable','Executável MySQL',inherit_default=True),Field('host','Host','127.0.0.1',True),Field('port','Porta',inherit_default=True),Field('socket','Socket MySQL (opcional)',''),Field('database','Banco padrão (opcional)',''),Field('connect_timeout','Timeout de conexão (segundos)',inherit_default=True),Field('timeout_seconds','Timeout de execução (segundos)',inherit_default=True),Field('allow_client_commands','Permitir comandos do cliente',inherit_default=True),Field('vault_profile','Perfil KeePass',required=True),Field('vault_entry_path','Entrada KeePass',required=True),Field('username_field','Campo do usuário','username'),Field('password_field','Campo da senha','password'))
...
def main():
 ...
 interactive_configure(root=root,path=config_path(root),title='MySQL',subtitle='Perfis de acesso a bancos MySQL',integration='MySQL',defaults=DEFAULTS,fields=PROFILE_FIELDS,default_fields=DEFAULT_FIELDS,test=profile_test)
