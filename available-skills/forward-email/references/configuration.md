...
O wrapper recebe `<CODEX_HOME>/configs/forward-email.toml` por `--config` e um perfil explícito por `--profile`. A seção `[forward_email]` define a API e o domínio padrão. O domínio também pode ser informado em `params.domain`.
